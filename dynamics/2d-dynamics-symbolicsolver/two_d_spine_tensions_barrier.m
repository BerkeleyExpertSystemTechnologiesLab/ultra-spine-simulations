...
t81 = sqrt(t75);
tensions_sub_barrier = [(t21.*5.0e1-u1.*2.0e3+t22.*t32.*(5.0./2.0))./(exp(t21.*-2.5e4+u1.*1.0e6-t22.*t32.*1.25e3+5.0)+1.0);(t47.*5.0e1-u2.*2.0e3+t48.*t49.*(5.0./2.0))./(exp(t47.*-2.5e4+u2.*1.0e6-t48.*t49.*1.25e3+5.0)+1.0);(t67.*5.0e1-u3.*2.0e3+1.0./sqrt(t58).*t66.*(5.0./4.0))./(exp(t67.*-2.5e4+u3.*1.0e6-t66.*1.0./sqrt(t55+t50.^2.*4.0).*6.25e2+5.0)+1.0);(t81.*5.0e1-u4.*2.0e3+t76.*t80.*(5.0./4.0))./(exp(t81.*-2.5e4+u4.*1.0e6-t76.*t80.*6.25e2+5.0)+1.0)];
