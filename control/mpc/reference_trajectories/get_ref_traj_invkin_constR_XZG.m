% get_ref_traj_invkin_constR_XZG.m
...
% Unlike the original invkin_XZG, this script moves the vertebrae at a constant radius, not a varying radius.

function [traj, num_points] = get_ref_traj_invkin_constR_XZG(tetra_vertical_spacing, num_points, direction)
% Inputs:
...
% from the origin. Call that angle beta.
% The radii are constant, e.g. r(t) = r0 \forall r.
...
for i=1:num_vertebrae
    % These positions are with reference to beta_j swept out from the +Z axis,
    % where a positive beta_j is clockwise.
    x_ref(:,i) = (tetra_vertical_spacing * i) .* sin(beta(:,i));
    z_ref(:,i) = (tetra_vertical_spacing * i) .* cos(beta(:,i));
end

% Also, define the rotation of each tetra around its own axis. This is variable gamma, state 5 out of 12 for each vertebra.
% We want the vertebrae to be normal to their radius. With some geometry,
% we find that the angle in the global coordinate frame (sweep angle beta_j) is equal to the
% rotation of each vertebra around its local coordinate system (rotation gamma_j).
% So, just copy over the beta angles.
