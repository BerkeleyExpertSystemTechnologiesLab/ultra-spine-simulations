...
% Revised in 2016-12 from Drew's code 2015 for 3-dimensional to 2-dimensional version,
% x_ref = (c1*beta + z_0)*sin(beta), z_ref = (c1*beta + z_0)*cos(beta),
% theta_ref = c2*beta
% Added derivatives of [x, z, theta] as later 3 states too.

% Modified in 2017-06 to match with new 3D inverse-kinematics:
% x_ref = z_0*sin(beta), z_ref = z_0*cos(beta),
% theta_ref = beta.
...
function [traj, num_points] = get_ref_traj_invkin_XZG_new(tetra_vertical_spacing, num_points, direction,dt)
% Inputs:
