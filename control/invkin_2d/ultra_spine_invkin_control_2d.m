% ultra_spine_invkin_control_2d.m
%
...
%
% This script tests the following situation:
% Apply the inverse kinematics control inputs to the system directly, in
% open loop, and compare the output trajectory with the goal from inv kin
% (should reach the same equilibrium point.)
...
disp('ULTRA Spine InvKin Control 2D')
...
% For the inverse kinematics controller,
% let's do a very long wait in between applications of control.
% One second is a very long time but should be good for testing.
opt_params.dt = 0.01;
...
% Also here, assume the system starts at the beginning of xi_traj.
xi_cl(:,1) = xi_traj(:,1);
...
% opt_params.xip1 = xi_traj(:,2);
    ...
    % For the inverse kinematics controller: 
    % Simply apply the corresponding u_traj consecutively.
    % Number sub-steps for Euler integration:
    euler_steps = 1000;
    xi_kp1 = simulate_2d_spine_dynamics(opt_params.xi, u_traj(:,i), ...
        opt_params.dt, euler_steps, dyn_type);
